...
/** @brief A cmd_index has this bit set if it's an application command. */
#define IS_APP_CMD 0x80

/**
 * @brief Index identifier for a command. 
 */
enum cmd_index {
...
/**
 * @var CMD_ERROR_COMMAND_INHIBIT_DAT_BIT_SET 
 * Command which uses DAT line found DAT line high (i.e. a data transfer command or a 
 * command which uses busy signal, since the busy signal is transmitted on the data line).
 *
 * @var CMD_ERROR_INTERRUPT_ERROR 
 * One of the error interrupts was flagged.
 *
 * @var CMD_ERROR_CARD_STATUS_ERROR 
 * A command with a card status response had an error bit set in the card status.
 *
 * @var CMD_ERROR_RESPONSE_CONTENTS 
 * Contents of command response not as expected.
 */
enum cmd_error {
	CMD_ERROR_NONE,
 ...
 *
 * @param idx Index of a standard (not application) command (unless this is being called 
 *	      from sd_issue_acmd()) 
 * @param args Arguments optionally used by the command
 */
enum cmd_error sd_issue_cmd(enum cmd_index idx, uint32_t args);
/**
 * @param idx Index of an application command
 * @param rca Relative card address used as argument to CMD55
 *
 * @return CMD_ERROR_RESPONSE_CONTENTS if the APP_CMD bit was not set in the CMD55 response.
 */
enum cmd_error sd_issue_acmd(enum cmd_index idx, uint32_t args, int rca);

/**
 * @brief Verify the card can operate on the 2.7-3.6V host supply voltage.
 *
 ...
 *
 * @param[out] card_capacity_support_out Card capacity support, whether card is SDHC/SDXC (true) or SDSC (false).
 *					 Only valid on success.
 *
 * @return CMD_ERROR_RESPONSE_CONTENTS voltage range not supported in card's OCR register
 * @return CMD_ERROR_GENERAL_TIMEOUT if the card did not power up in 1 second
 */
enum cmd_error sd_issue_acmd41(bool *card_capacity_support_out);

/**
 * @brief Publish a new relative card address for the card in out-param rca_out.
 */
enum cmd_error sd_issue_cmd3(int *rca_out);

/**
 * @brief Toggle a card between the stand-by and transfer states dependent on RCA (see phys spec 
...
/**
 * @brief Card status response from RESPONSE_R1_NORMAL and RESPONSE_R1B_NORMAL_BUSY. 
 */
struct card_status {
	bits_t reserved1 : 3;
	bits_t ake_seq_error : 1;
	bits_t reserved2 : 1;
	bits_t app_cmd : 1;  /**< Signals whether the next issued command is expected to be an application command */
	bits_t reserved3 : 2;
...
/**
 * @brief Get the addressed card's status.
 */
enum cmd_error sd_issue_cmd13(int rca, struct card_status *cs_out);

/**
 * @brief Set the addressed card's data bus width.
 * @param four_bit True for 4-bit data bus width, false for 1-bit.
 */
enum cmd_error sd_issue_acmd6(int rca, bool four_bit);

/**
 * @brief Read a single block (CMD17) or multiple blocks (CMD18) of size SD_BLKSZ 
 *	  from the SD card into RAM.
 *
 * @param ram_dest_addr 4-byte aligned destination address in RAM to copy read data to
 * @param sd_src_addr Source SD card address to read data from. If the card is SDSC this 
 *		      should be a byte unit address. If the card is SDHC or SDXC this should 
 *		      be a block unit address (LBA).
...
/** @brief Get the SD card to send its SCR register. */
